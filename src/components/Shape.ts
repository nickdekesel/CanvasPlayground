...
export class Line extends Shape {
  isInside(position: Position): boolean {
    ...
    const end = { x: this.position.x + this.width, y: this.position.y + this.height };
    
    const distance = Math.abs((position.y - end.y) * start.x -
      (position.x - end.x) * start.y + position.x * end.y - position.y * end.x) /
      Math.sqrt(Math.pow((position.y - end.y), 2) + Math.pow((position.x - end.x), 2));
    
    if (distance > tolerance) {
      return false;
    }

    const dotProduct = (position.x - start.x) * (end.x - start.x) + (position.y - start.y)*(end.y - start.y);
    if (dotProduct < 0) {
      return false;
    }
